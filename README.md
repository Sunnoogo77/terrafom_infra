...
- Backend Terraform distant : le workflow init utilise un backend Azure Storage. Secrets requis : `TF_STATE_RESOURCE_GROUP`, `TF_STATE_STORAGE_ACCOUNT`, `TF_STATE_CONTAINER`, plus la clé par défaut `terraform-dev.tfstate` (configurable via `TF_BACKEND_KEY`). Le Service Principal de `AZURE_CREDENTIALS` doit avoir accès au RG/Storage/Container.
- Variables/Secrets CI pour Terraform : `TF_VAR_env` (dev), `TF_VAR_sql_admin_login`, `TF_VAR_sql_admin_password`, `TF_VAR_frontend_repository_url` (peut être en var GitHub), backend : `TF_BACKEND_RESOURCE_GROUP_NAME`, `TF_BACKEND_STORAGE_ACCOUNT_NAME`, `TF_BACKEND_CONTAINER_NAME`, `TF_BACKEND_KEY`.
- Rôles à attribuer au Service Principal (Azure AD) utilisé par `AZURE_CREDENTIALS` : sur la souscription ou le RG cible → `Contributor` (management plane). Sur le Storage Account du backend → `Storage Blob Data Contributor` (data plane) pour permettre la lecture/écriture du state et la création du container si absent.
- Permissions Azure AD pour la création des groupes (module `modules/groups`) : l’identité CI doit avoir un rôle Directory (ex. `Directory.ReadWrite.All` via AppRole ou `Privileged Role Administrator`) autorisant la création de groupes. Sans ces droits, la création échoue en 403 ; dans ce cas, créer les groupes manuellement ou donner le droit requis avant d’exécuter le pipeline.
